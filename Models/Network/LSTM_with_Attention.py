...
def LSTM_with_Attention(Input, max_time, n_input, lstm_size, attention_size, keep_prob, weights_1, biases_1, weights_2, biases_2):
    '''
    ...
    # One layer Attention-based LSTM Model
    Input = tf.reshape(Input, [-1, max_time, n_input])
    lstm_cell = tf.contrib.rnn.BasicLSTMCell(num_units=lstm_size)
    lstm_drop = tf.contrib.rnn.DropoutWrapper(cell=lstm_cell, input_keep_prob=keep_prob)
    outputs, final_state = tf.nn.dynamic_rnn(cell=lstm_drop, inputs=Input, dtype=tf.float32)
